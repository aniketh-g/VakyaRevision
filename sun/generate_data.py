...
SIDE_YR =  365.256363004 # Sidereal year

#####-------- Set Initial year, averaging window, and number of steps -------#####
startyear = 2029 # AD
numyears = 60    # Averaging window
steps = 1000     # Number of readings per year
target_year = 2029 # Sidereal single year

# Configure Parameters
COMPUTE_PRESENT_AN = sys.argv[1]
COMPUTE_PRESENT_SI = sys.argv[2]
COMPUTE_HISTORICAL_AN = sys.argv[3]
...
def get_mean_anom_longs(startyear):
    ...
    datas = [[swe.calc(jd+year*ANOM_YR+i*ANOM_YR/steps, swe.SUN, swe.FLG_SIDEREAL)[0][0] \
            ...
            - swe.get_ayanamsa_ex(tjdet=jd+year*ANOM_YR+i*ANOM_YR/steps, flags=0)[1]) \
                for i in range(steps)] for year in range(numyears)]

    return [circmean([data[i] for data in datas], high=360, low=0) for i in range(steps)]

if COMPUTE_PRESENT_AN == 'T':
    print('Computing average anomalistic longitudes from AD %d to %d'%(startyear, startyear+numyears))
...
if COMPUTE_PRESENT_SI == 'T':
    ...
    # mandocca_true = swe.nod_aps(jd, swe.SUN)[3][0] - swe.get_ayanamsa_ex(tjdet=jd, flags=0)[1]
    # print('Mandocca on jd =%f\n'%mandocca_true)
    swelongs_si = [swe.calc(jd+i*ANOM_YR/steps, swe.SUN, swe.FLG_SIDEREAL)[0][0] for i in range(steps)]

    mandocca_true = circmean([swe.nod_aps(jd+i*SIDE_YR/steps, swe.SUN)[3][0] - swe.get_ayanamsa_ex(tjdet=jd+i*SIDE_YR/steps, flags=0)[1] for i in range(steps*numyears)], high=360, low=0)
    ...
    print('swelongs_si starting on jd = %f (%s) at sidereal longitude = %f'%(jd, swe.revjul(jd), swelongs_si[0]))
...
if COMPUTE_HISTORICAL_AN == 'T':
    ...
    for year in range(-3000, 2001, 500):
        print('Computing average anomalistic longitudes from AD %d to %d, step interval = ANOM_YR/%d'%(year, year+numyears, steps))
